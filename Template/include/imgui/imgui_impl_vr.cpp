...
static GLuint		g_GuiTexture[4] = {0,0,0,0};
...
void ImGui_Impl_VR_SetSticky(bool sticky)
{
	g_Sticky = sticky;
}
...
{
	//return SDL_GetClipboardText(); //any equivalent here?
	return 0;
}
...
void ImGui_Impl_VR_ProcessEvent(vr::VREvent_t* event)
{
	...
	{
		ImGui::SetInternalState(state[g_ActiveTexture]);		
	}
	...
	if((event->eventType == vr::VREvent_KeyboardDone) && (InputTextBuf != 0) && (InputTextBufSize > 0)) 
	{
		...
		for(int i=0; i<InputTextBufSize; i++)
		{
...
void ImGui_Impl_VR_ProcessController(vr::TrackedDeviceIndex_t device, vr::VRControllerState_t* state, glm::mat4& M)
{
	...
	if(device == right_controller)
	{
		...
		if(state->ulButtonTouched & ButtonMaskFromId(vr::k_EButton_SteamVR_Touchpad))
		{
			...
			{
				g_MouseWheel += new_axis - last_axis;
			}
			last_axis = new_axis;
			g_AxisPos[1] = ImVec2(state->rAxis[0].x, state->rAxis[0].y);
		}
...
void ImGui_Impl_VR_Poll3dMouse()
{
	...
	for(int i=0; i<4; i++)
	{
		ImGui::SetInternalState(state[i]);
		ImGuiIO& io = ImGui::GetIO();
		io.MouseDrawCursor = true;
		io.MousePos = ImVec2(500.0f, 500.0f);
	}
	return;
	...
	for(int i=0; i<4; i++)
	{
		...
		{
			ImGuiIO& io = ImGui::GetIO();
			io.MousePos = ImVec2(1024.0f*(0.5f*pt.x + 0.5f), 1024.0f - 1024.0f*(0.5f*pt.y + 0.5f)); 
			io.MouseDrawCursor = true; //mouse still not showing
			g_ActiveTexture = i;
		}
	}
}
...
void    ImGui_Impl_VR_InvalidateDeviceObjects()
{
	...
	if(g_GuiTexture)
	{
		glDeleteTextures(4, g_GuiTexture);
	}
...
bool ImGui_Impl_VR_Init(vr::IVRSystem* hmd, int w, int h)
{
	...
	const ImColor tex_colors[4] = {ImColor(1.0f, 0.0f, 0.0f, 1.0f), ImColor(0.0f, 1.0f, 0.0f, 1.0f), ImColor(0.0f, 0.0f, 1.0f, 1.0f), ImColor(1.0f, 0.0f, 1.0f, 1.0f)};

	for(int i=0; i<4; i++)
	{
		if(i==0)
		{
			state[0] = ImGui::GetInternalState();
		}
		else
		{
			state[i] = (void*) new char[ImGui::GetInternalStateSize()];
			ImGui::SetInternalState(state[i], true);
		}

		ImGuiStyle& style = ImGui::GetStyle();
		style.Colors[ImGuiCol_TitleBg] = tex_colors[i];
...
void  ImGui_Impl_VR_Render(int i)
{
		...
		static bool WantTextInputLastFrame[4] = {false, false, false, false};
		ImGuiIO& io = ImGui::GetIO();
