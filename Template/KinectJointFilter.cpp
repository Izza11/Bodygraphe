...
//--------------------------------------------------------------------------------------
void FilterDoubleExponential::Update(IBody* const pBody, Joint* joints)
{
	//assert(pBody);
	std::cout << "INSIDE UPDATE 1" << std::endl;
	...
	for (INT i = 0; i < JointType_Count; i++)
	{
		...
		joints[i] = Update(joints, i, SmoothingParams);
		//std::cout << "x: " << (float)joints[i].Position.X << "   y: " << (float)joints[i].Position.Y << "   z: " << (float)joints[i].Position.Z << std::endl;
	}
}
...
Joint FilterDoubleExponential::Update(Joint joints[], UINT JointID, TRANSFORM_SMOOTH_PARAMETERS smoothingParams)
{
	...
	XMStoreFloat4(&v2F, m_pFilteredJoints[JointID]);
	joint.Position.X = v2F.x;
	joint.Position.Y = v2F.y;
	joint.Position.Z = v2F.z;
